...
from src.core import autostart, config, fonts, reminders
...
QCheckBox::indicator:checked {{ background: #F4C77B; }}
QLineEdit, QComboBox {{
    border: 2px solid #5C4221;
    border-radius: 0px;
    padding: 4px 8px;
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent: QWidget | None = None, initial_tab: int = 0) -> None:
        ...
        self.setStyleSheet(_DIALOG_QSS_TEMPLATE.format(family=fonts.family_css()))
    ...
    def _build_ai_tab(self) -> QWidget:
        ...
        self._edt_api_key = QLineEdit(ai_cfg.get("api_key", ""))
        ...
        layout.addStretch(1)
        layout.addWidget(QLabel("Key 仅保存在本机配置文件，不会上传。"))
